    ...
    public function get_article($url)
    {
        if($url) {
            ...
            foreach ($result_img as $res) {
                if($i++==2) echo '<img width="700" src="http://levif.be'.$res->getAttribute('src').'"/>';
            }
    ...
    {    
        ...
        try {
            ...
            foreach ($results as $result) {
                ...
                $description = $result->childNodes->item(2);
                
                $inject_script = 'jQuery("#url_data").val("'.$result->childNodes->item(1)->textContent.'");jQuery("#page_position").val("'.$i.'");document.getElementById("url_form").submit();';
                
                echo "<article id='".($i++)."'class='forecast' onclick='".$inject_script."'>";
                echo "<h3>".$titre->nodeValue."</h3>";
                echo "<article class='day-forecast'><p>".$description->nodeValue."</p></article>";
                echo "</article>";
            }
            echo '</div>';
