		...
		private function doActions() {
			...
			if ( !willQuit) 
			{
				try {
					...
					// barre de vie
					var barreVie = Global.ROOT_CLIP.getChildByName("mc_fiole");
					var ratio = Global.PLAYER.life / 999; trace(ratio + ' / ' + barreVie.height);
					barreVie.height * ratio;
			...
			try 
			{
				...
				switch (this.count) { // show admob interstitial if user is awaiting too long
					...
					case 3:
						...
						if (Global.PLAYER.l_hand.currentFrameLabel == 'Fist') Global.PLAYER.freePosition();
						break;
